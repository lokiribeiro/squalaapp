  ...
  constructor($rootScope, $scope, $mdToast, $timeout, $element, $q, $mdSidenav, $log, $mdDialog, $state, Upload){
      ...
      users(){
        ...
        userBalances.forEach(function(userBalance) {
          ...
          for(i=0;i<counter;i++){
            if(userBalance.statements[i].appliedToMonth == $scope.currentMonth){
              $scope.notclearedForRC = 'yes';
            }
...
app.component('rcgeneration', {
    templateUrl: 'client/components/headmasterProfile/headmasterCollect/rcgeneration/rcgeneration.html',
    controllerAs: 'rcgeneration',
    controller: RcgenerationCtrl
})
