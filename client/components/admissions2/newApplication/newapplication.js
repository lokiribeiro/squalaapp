...
class AdmissionsnewapplicationCtrl{
  ...
  constructor($scope, $timeout, $reactive, $mdSidenav, $log, $mdDialog, $state){
      ...
      $scope.createUser = function(details) {
        ...
        Meteor.call('createApplicantFromAdmin', detail.firstname, detail.middlename, detail.lastname, detail.email, detail.status, detail.application, detail.branch, detail.encodedBy, detail.photo, detail.encoderName, detail.createdAt, detail.fees._id, detail.fees.feesName, progress, function(err, detail) {
              var detail = detail;
              console.log(detail);
                if (err) {
