  ...
  constructor($rootScope, $scope, $mdToast, $timeout, $mdSidenav, $log, $mdDialog, $state, Upload){
    ...
    $scope.announceClick = function($event, branch, firstname, lastname, ID, userType, oldBranchId) {
      ...
      $scope.associateRole = function($event, firstname, lastname, userId, roleId, rolename) {
        ...
        $scope.items = [
          { name: "Upload teaching materials", icon: "../../assets/img/white_addlesson24.svg", direction: "left" }
        ];

        $scope.openDialog = function($event, item) {
          // Show the dialog
          if(item.name == 'Upload teaching materials'){
          $mdDialog.show({
            ...
            controller: LessonmaterialsCtrl,
            template: '<uploadmaterials></uploadmaterials>',
            targetEvent: $event
          });
        }
      }
...
app.component('lessonmaterials', {
    templateUrl: 'client/components/classroom/lessonmaterials/lessonmaterials.html',
    controllerAs: 'lessonmaterials',
    controller: LessonmaterialsCtrl
})
