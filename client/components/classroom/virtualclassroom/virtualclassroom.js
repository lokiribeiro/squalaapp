...
class virtualclassroomCtrl{

  constructor($rootScope, $scope, $mdToast, $timeout, $mdSidenav, $log, $mdDialog, $state, Upload){
      'ngInject';

      $scope.userID = Meteor.userId();
      $scope.classID = $rootScope.classID;
      $scope.courselinkID = $rootScope.courselinkID;

      $scope.subscribe('classlists', function () {
          return [$scope.getReactively('classID')];
      });
      ...
      $scope.helpers({
        classlists(){
          var classID = $scope.getReactively('classID');
          var selector = {_id : classID};
          var classlists = Classlists.find(selector);
          console.info('classlists', classlists);
          var counter = classlists.count();
          console.log(counter);
          return classlists;
        }
      });//helpers
    ...
    $scope.announceClick = function($event, branch, firstname, lastname, ID, userType, oldBranchId) {
      ...
      var branchDetails = classlists.find(selector);
      ...
      if(counter > 0){
          branchDetails.forEach(function(branchDetail){
            if(branchDetail.classlists_schooladmin == profileID){
              var selector = {_id: oldId};
              var modifier = {$set: {
                classlists_schooladmin: '',
                classlists_schooladminname: ''
              }};
              var removeAdmin = classlists.update(selector,modifier);
            }
          })
      }
      ...
      $scope.associateRole = function($event, firstname, lastname, userId, roleId, rolename) {
        ...
        $scope.items = [
          { name: "New Assignment", icon: "../../assets/img/white_addclass24.svg", direction: "left" }
        ];
        ...
        $scope.openResp = function () {
          ...
          var courselinkID = $scope.courselinkID;
          $state.go('ClassroomResources', {stateHolder : 'Classroom', userID : Meteor.userId(), classID : classID, courselinkID : courselinkID});
        }

        $scope.openSchool = function () {
          var classID = $scope.classID;
          $state.go('Headmasterschool', {stateHolder : 'Headmaster', userID : Meteor.userId(), classID : classID});
        }

    }
}

app.component('virtualclassroom', {
    templateUrl: 'client/components/classroom/virtualclassroom/virtualclassroom.html',
    controllerAs: 'virtualclassroom',
    controller: virtualclassroomCtrl
})
